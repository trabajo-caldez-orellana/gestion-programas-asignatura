...
from backend.common.mensajes_de_error import MENSAJE_NO_HAY_ANIOS_FUTUROS
...
class TestObtenerAnioAcademicoSiguiente(TestCase):
    ...
    def test_hay_dos_anios_futuros(self):
        AnioAcademico.objects.create(
            fecha_inicio=FECHA_FIN_ANIO_FUTURO + timezone.timedelta(days=1),
            fecha_fin=FECHA_FIN_ANIO_FUTURO + timezone.timedelta(days=181),
        )

        anio = self.servicio_anio_academico.obtener_anio_academico_siguiente()
    ...
    def test_probar_despues_de_media_noche_en_cambio_de_anio(self):
        dia_de_referencia = FECHA_INICIO_ANIO_CERRADO + timezone.timedelta(days=1)
    ...
    def test_no_se_crearon_anios(self):
        ...
        with self.assertRaises(ValidationError) as contexto:
            self.servicio_anio_academico.obtener_anio_academico_siguiente()
    ...
    def test_no_hay_anios_siguientes(self):
        self.anio_futuro.delete()
