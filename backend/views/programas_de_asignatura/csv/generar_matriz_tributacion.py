...
class GenerarMatriz(APIView):
    ...
    def get(self, request, id_carrera, id_plan_de_estudio):
        try:
            ...
            return Response(
                {"error": {"carrera": [MENSAJE_ID_INEXISTENTE]}},
        ...
        try:
            ...
            return Response(
                {"error": {"plan_de_estudio": [MENSAJE_ID_INEXISTENTE]}},
                status=HTTP_400_BAD_REQUEST,
            )
        ...
        if carrera.id != plan_de_estudio.carrera_id:
            return Response(
                {"error": {"__all__": [MENSAJE_CARRERA_PLAN]}},
                status=HTTP_400_BAD_REQUEST,
            )

        obtener_datos_matriz = ObtenerDatosMatriz()
        try:
            datos_matriz = obtener_datos_matriz.obtener_datos_matriz_de_tributacion(
        ...
        response = HttpResponse(
            ...
            headers={
                "Content-Disposition": 'attachment; filename="matriz-tributacion.csv"'
            },
        )
