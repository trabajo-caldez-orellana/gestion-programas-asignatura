...
class ObtenerDatosMatriz:
    ...
    def obtener_datos_matriz_de_tributacion(
        self,
        carrera: Carrera,
        ...
        estandar_actual = self.servicio_estandar.obtener_estandar_activo_de_carrera(
            carrera
        )

        # Obtengo todos los descriptores del estandar
        descriptores = Descriptor.objects.filter(
            estandar__in=[estandar_actual.id], tipo=TipoDescriptor.DESCRIPTOR
        )
        ejes_transversales = Descriptor.objects.filter(
            estandar__in=[estandar_actual.id], tipo=TipoDescriptor.EJE_TRANSVERSAL
        )
        actividades_reservadas = ActividadReservada.objects.filter(
            estandar_id=estandar_actual.id
        )

        # Obtengo todas las asignaturas del plan actual para el semestre actual!
        asignaturas = Asignatura.objects.filter(
            planes_de_estudio__in=[plan_de_estudio.id]
        )
        ...
        cabecera_csv = [""] + [
            programa.asignatura.denominacion for programa in programas_de_asignatura
        ]
        filas_matriz.append(cabecera_csv)
        separacion_csv = ["Descriptores"] + [
            "" for _ in range(len(programas_de_asignatura))
        ]
            ...
            for programa in programas_de_asignatura:
                try:
                    eje_del_programa = ProgramaTieneDescriptor.objects.get(
                        descriptor_id=eje.id, version_programa_asignatura_id=programa.id
                    )
